...
export const FacultyTab: React.FC = () => {
  ...
  // SE&LD Checker Verification modal state
  ...
  // Strict rule: Only teachers who registered via portal, approved by admin, and verified via SE&LD checker (https://checker.sindheducation.gov.pk/)
  // All other unapproved or default mock teachers are strictly removed.
  const approvedTeachers = useMemo(() => {
    return teachers.filter(
      (t) => t.status === 'approved' && (t.verifiedBadgeIssued || t.seldVerified)
    );
  }, [teachers]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* SE&LD Official Checker & Faculty Verification Authority Banner */}
      <div className="bg-gradient-to-r from-slate-950 via-emerald-950 to-slate-900 text-white rounded-3xl p-6 sm:p-7 shadow-xl border-2 border-emerald-600/60 relative overflow-hidden">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
            <div className="w-18 h-18 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-emerald-800 to-teal-900 border-2 border-amber-400 shadow-xl shrink-0 flex items-center justify-center relative">
              <ShieldCheck className="w-10 h-10 text-amber-400" />
              <span className="absolute -bottom-1 -right-1 bg-emerald-500 text-slate-950 text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-xs">
                VERIFIED
              </span>
            </div>
            <div className="space-y-1.5">
              <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-400/40 text-emerald-200 text-[11px] font-extrabold uppercase tracking-wider">
                <ShieldCheck className="w-3.5 h-3.5 text-amber-300" />
                <span>SE&LD Institutional Verification Protocol</span>
              </div>
              <h2 className="text-xl sm:text-2xl font-black tracking-tight text-white">
                Government Verified Faculty Directory
              </h2>
              <p className="text-xs text-emerald-200 font-semibold">
                School Education & Literacy Department, Govt. of Sindh • SEMIS 406020752
              </p>
              <p className="text-xs text-slate-300 max-w-xl pt-0.5 leading-relaxed">
                By official mandate, all default/mock teacher profiles are removed. Teachers appear in this Faculty Section exclusively after registering via the <strong>Teacher Portal</strong>, completing administrative scrutiny, and obtaining an authentic <strong>SE&LD Verified Badge</strong> linked with the official checker portal.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto shrink-0">
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3.5 sm:p-4 border border-white/20 text-center w-full sm:w-auto min-w-[170px]">
              <span className="text-[10px] uppercase font-extrabold text-amber-300 block mb-1">
                SE&LD Central Checker
              </span>
              <a
                href="https://checker.sindheducation.gov.pk/"
                target="_blank"
                rel="noopener noreferrer"
                className="bg-emerald-950/80 hover:bg-emerald-900 rounded-xl p-2.5 border border-emerald-700/60 block text-left space-y-1 transition group"
              >
                <div className="flex items-center gap-1.5 text-[11px] font-bold text-emerald-200 group-hover:text-amber-300">
                  <ExternalLink className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                  <span>checker.sindheducation.gov.pk</span>
                </div>
          ...
          {filteredTeachers.map((teacher) => (
            ...
            >
              ...
              <div className="p-6 pt-0 flex-1 flex flex-col -mt-10">
                ...
                {/* SE&LD Verified Badge & Checker Link */}
                <div className="mt-3.5 p-3 rounded-xl bg-gradient-to-br from-emerald-950 via-emerald-900 to-teal-950 text-white border-2 border-amber-400 shadow-md space-y-2">
                  ...
                  <div className="text-[10px] font-mono text-emerald-200/90 truncate flex items-center justify-between">
                    <span className="truncate mr-1">ID: {teacher.verifiedBadgeId || `SELD-VERIFIED-406020752-${teacher.pid}`}</span>
                    <span className="text-[9px] text-amber-300 font-bold shrink-0">Biometric Matched</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setSelectedTeacherForVerificationModal(teacher)}
                    className="w-full py-1.5 px-2.5 rounded-lg bg-emerald-800/90 hover:bg-emerald-700 text-amber-300 hover:text-white text-[11px] font-bold border border-emerald-600 transition flex items-center justify-center gap-1.5 shadow-xs"
                  >
                    <ExternalLink className="w-3.5 h-3.5 text-amber-400 shrink-0" />
      ...
      {viewMode === 'list' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm divide-y divide-slate-100 overflow-hidden">
          {filteredTeachers.map((teacher) => (
            <div
              key={teacher.id}
              className="p-4 sm:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-slate-50/80 transition"
            >
              ...
              {/* SE&LD Verified Badge in List View */}
              <div className="flex flex-col sm:flex-row md:flex-col items-start md:items-end gap-1.5 shrink-0">
                <div className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-gradient-to-r from-emerald-950 to-teal-950 text-white border border-amber-400 text-xs font-bold shadow-xs">
                  <ShieldCheck className="w-4 h-4 text-amber-400 shrink-0" />
                  <span className="text-amber-300">SE&LD Verified</span>
      ...
      {filteredTeachers.length === 0 && (
        <div className="text-center py-16 bg-white rounded-3xl border-2 border-dashed border-emerald-300 shadow-sm p-8 space-y-5">
          <div className="w-20 h-20 rounded-3xl bg-emerald-50 text-emerald-800 flex items-center justify-center mx-auto border-2 border-emerald-300 shadow-xs">
          ...
          {approvedTeachers.length === 0 ? (
            <div className="space-y-4 max-w-lg mx-auto">
              <span className="inline-block px-3 py-1 rounded-full bg-amber-100 text-amber-900 border border-amber-300 text-xs font-bold uppercase tracking-wider">
                Strict SE&LD Verification Required
              </span>
              <h3 className="text-xl font-black text-slate-900">
                No Verified Faculty Members Approved Yet
              </h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                By default, mock and unverified teachers are removed. Teachers will display in this Faculty Section only after submitting their registration via the <strong>Teacher Portal</strong>, obtaining official <strong>Headmaster / Admin approval</strong>, and receiving a verified badge authenticated with the Sindh Education Checker (<strong>https://checker.sindheducation.gov.pk/</strong>).
              </p>
              <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
                <button
                  onClick={() => setShowRegisterModal(true)}
                  className="w-full sm:w-auto px-6 py-3 rounded-xl bg-emerald-800 hover:bg-emerald-700 text-white font-black text-xs shadow-md transition inline-flex items-center justify-center gap-2"
                >
                  <UserPlus className="w-4 h-4 text-amber-300" />
                  <span>Join Faculty / Register via Teacher Portal</span>
                </button>
                <a
                  href="https://checker.sindheducation.gov.pk/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full sm:w-auto px-5 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs border border-slate-300 transition inline-flex items-center justify-center gap-2"
                >
                  <ExternalLink className="w-4 h-4 text-emerald-700" />
                  <span>Sindh Education Checker</span>
                </a>
              </div>
            </div>
          ) : (
            <div className="space-y-3 max-w-md mx-auto">
              ...
              <p className="text-xs text-slate-500">
                No verified teachers match the current search keyword "{searchTerm}" or selected filters.
              </p>
              <button
                onClick={resetFilters}
          ...
          <div className="bg-white rounded-2xl max-w-xl w-full border border-slate-200 shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmitTeacher} className="p-6 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {/* Timetable Auto-Assign Engine Subject & Class Mapping */}
                <div className="sm:col-span-2 bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-black text-slate-900 uppercase tracking-wide">
                        Timetable Engine: Subject & Class Mapping (Optional for Auto-Scheduler)
                      </p>
                      <p className="text-[11px] text-slate-500">
                        Select the subjects and classes you teach. The automated timetable engine will distribute your periods across Classes 1-10 without conflicts.
                      </p>
                    </div>
                  </div>

                  <div>
                    <label className="block text-[11px] font-bold text-slate-600 mb-1.5">Assigned Subjects:</label>
                    <div className="flex flex-wrap gap-1.5">
                      {['Mathematics', 'Physics', 'Computer Science', 'English', 'General Science', 'Chemistry', 'Biology', 'Sindhi', 'Social Studies', 'Islamiat / Ethics'].map((subj) => {
                        const isSelected = formData.assignedSubjects.includes(subj);
                        return (
                          <button
                            key={subj}
                            type="button"
                            onClick={() => {
                              setFormData({
                                ...
                                assignedSubjects: isSelected
                                  ? formData.assignedSubjects.filter((s) => s !== subj)
                                  : [...formData.assignedSubjects, subj],
                              });
                            }}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-600 mb-1.5">Assigned Classes (Classes 1 to 10):</label>
                    <div className="flex flex-wrap gap-1.5">
